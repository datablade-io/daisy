...
clickhouse-test -q /clickhouse-tests-env/queries -j$(($(nproc)/2)) --no-stateful --testname --shard --zookeeper --print-time $@ 2>&1 | ts '%Y-%m-%d %H:%M:%S' | tee /tests_output/${TEST_TAG}_statelest_test_result.txt
