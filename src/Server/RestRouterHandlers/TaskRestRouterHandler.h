...
{
public:
    explicit TaskRestRouterHandler(Context & query_context_) : RestRouterHandler(query_context_, "Task") { }
    ~TaskRestRouterHandler() override { }

private:
    String executeGet(const Poco::JSON::Object::Ptr & payload, Int32 & http_status) const override;
    static Poco::JSON::Array getResult(std::vector<TaskStatusService::TaskStatusPtr> & tasks);
};
