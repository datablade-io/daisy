...
void RestHTTPRequestHandler::handleRequest(HTTPServerRequest & request, HTTPServerResponse & response)
{
    ...
    try
    {
        ...
        if (!router_handler->outputStreaming())
        {
        ...
        {
            /// Send output in streaming mode, the router_handler will send data in execute(...), no need to call response.send
            router_handler->execute(request, response, http_status);
        }
