...
TEST(CheckRecordSerializationDeserialization, Serder)
{
    ...
    IDistributedWriteAheadLog::Record r{IDistributedWriteAheadLog::ActionType::ADD_DATA_BLOCK, move(block)};

    std::vector<UInt8> data{IDistributedWriteAheadLog::Record::write(r)};

    auto rr = IDistributedWriteAheadLog::Record::read(reinterpret_cast<char *>(data.data()), data.size());
    EXPECT_EQ(r.action_type, rr.action_type);
