...
void DistributedMergeTreeBlockOutputStream::write(const Block & block)
{
    ...
    for (auto & current_block : blocks)
    {
        ...
        {
            LOG_TRACE(storage.log, "[async] write a block={} rows={} shard={} ... for query_status_poll_id={}", outstanding, current_block.block.rows(), current_block.shard, query_context->getQueryStatusPollId());
        ...
        {
            LOG_TRACE(storage.log, "[sync] write a block={} rows={} shard={} ... , and current committed={}", outstanding + 1, current_block.block.rows(), current_block.shard, committed);
        ...
        {
            LOG_TRACE(storage.log, "[fire_and_forget] write a block rows={} shard={} ...", current_block.block.rows(), current_block.shard);
        ...
        {
            /// ordered
            LOG_TRACE(storage.log, "[ordered] write a block rows={} shard={} ...", current_block.block.rows(), current_block.shard);
            ...
            LOG_TRACE(storage.log, "[ordered] write a block done");
        }
    }
}
...
void DistributedMergeTreeBlockOutputStream::writeCallback(const DWAL::AppendResult & result)
{
    ...
    LOG_TRACE(storage.log, "[sync] Writed block, and current committed={}, error={}", committed, errcode);
}
...
void DistributedMergeTreeBlockOutputStream::flush()
{
    ...
    while (1)
    {
        ...
        {
            LOG_DEBUG(storage.log, "[sync] write done, writed blocks={}, committed={}, error={}", outstanding, committed, errcode);
